...
\begin{note}
	\xfield{$E[X] = ?$}
	\xfield{$E[X] = \sum\limits_{s \in S} X(s) p(s)$}
\end{note}
...
\begin{note}
	\xfield{For $X$ and $Y$ independent random variables,\\
	$E[X\cdot Y] = $ ?}
	\xfield{$E[X\cdot Y] = E[X]\cdot E[Y]$}
\end{note}

\begin{note}
	\xfield{Give Markov inequality.}
	\xfield{$\mathbb{P}(X \geq a) \leq \frac{\mathbb{E}(X)}{a}$, for $a>0$}
\end{note}

\begin{note}
	\xfield{Give Chebychev inequality.}
	\xfield{$\mathbb{P}(|X-\mathbb{E}(X)| \geq a) \leq \frac{\textrm{Var}(X)}{a^2}$, for $a >0$ and where \\
